...
def parsePoint(line):
    ...
    Returns:
        LabeledPoint: The line is converted into a `LabeledPoint`, which consists of a label and
            features.
    """
    wordList = np.array(line.split(','))
    return LabeledPoint(wordList[0],wordList[1:])
...
onlyLabels = parsedDataInit.map(lambda labeledPt:labeledPt.label )
...
# TODO: Replace <FILL IN> with appropriate code
parsedData = parsedDataInit.map(lambda pt:LabeledPoint(pt.label-minYear,pt.features))
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return (labelsAndPreds.map(lambda (l,p):squaredError(l,p)).mean())**0.5
...
def twoWayInteractions(lp):
    ...
    """
    inter = map(lambda (x,y):x*y,list(itertools.product(lp.features,repeat=2)))
    return LabeledPoint(lp.label,np.hstack((lp.features,inter)))

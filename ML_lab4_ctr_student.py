...
aSparse = SparseVector(aDense.size,range(0,aDense.size),list(aDense))

bDense = np.array([0., 0., 0., 1.])
bSparse = SparseVector(bDense.size,range(0,bDense.size),list(bDense))
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    for index in range(len(rawFeats)):
        resultDict[OHEDict[rawFeats[index]]]=1.0
        
    return SparseVector(numOHEFeats,resultDict)
...
def parsePoint(point):
    ...
    """
    splitData = point.split(',')
    numFeatures = len(splitData)-1
    featureList = []
    for x in range(numFeatures):
        featureList.append((x,splitData[1+x]))
    return featureList
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    label = (point.split(','))[0]
    featureVector = oneHotEncoding(featureList,OHEDict,numOHEFeats)
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    resultDict = {}
    for index in range(len(rawFeats)):
        if(OHEDict.has_key(rawFeats[index])):
            resultDict[OHEDict[rawFeats[index]]] = 1.0
    return SparseVector(numOHEFeats,resultDict)
...
def parseHashPoint(point, numBuckets):
    ...
    featureList = parsePoint(point)
    label = (point.split(','))[0]
    featureVectorDict = hashFunction(numBuckets,featureList,False)
    featureVector = SparseVector(numBuckets,featureVectorDict)
    return LabeledPoint(label,featureVector)
...
def computeSparsity(data, d, n):
    ...
    """
    numNonZeroFeat = data.map(lambda lp:len([x > 0 for x in lp.features.values])).sum() 
    return float(numNonZeroFeat)/float(n*d)
